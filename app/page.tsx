...
const DEFAULT_SETTINGS: PlannerSettings = { taskTypes: BASE_TYPES, locations: BASE_LOCATIONS, defaultTaskType: '🧬 个人', defaultLocation: '🏠 家' };
...
function TaskCard({ task, color, now, dragging, landed, onOpen, onDragStart, onDragEnd, onDragOver, onDrop }: {
  ...
  return <article className={`task-card type-${color} priority-${task.priority} ${dragging ? 'is-dragging' : ''} ${landed ? 'is-landed' : ''} ${canDrag ? '' : 'is-locked'}`}
...
export default function Home() {
  ...
  const [impact, setImpact] = useState<{ title: string; subtitle: string } | null>(null);
  ...
  const choiceOptions = useMemo(() => {
    ...
    if (choiceField === 'taskType') return settings.taskTypes.map((item) => ({ value: item.value, label: item.value, color: item.color }));
  ...
  const moveTask = (id: string, status: Status) => {
    ...
    setLandedId(id);
    setImpact(status === 'completed' ? { title: 'MISSION CLEAR!', subtitle: '任务完成 · 战果已记录' } : status === 'inProgress' ? { title: 'MISSION START!', subtitle: '开始时间已自动记录' } : { title: 'MISSION RESET', subtitle: '任务已返回等待区' });
    setDropTarget('');
    setDraggingId('');
  };
  ...
  const openDatePicker = (field: DateFieldName) => {
    ...
    setPickerTime(`${String(value.getHours()).padStart(2, '0')}:${String(Math.floor(value.getMinutes() / 15) * 15).padStart(2, '0')}`);
  ...
  const [pickerHour, pickerMinute] = pickerTime.split(':').map(Number);
  ...
  return <main className={`app-shell view-${view} theme-${theme} page-motion-${pageMotion} page-direction-${pageDirection}`}>
    ...
    <aside className={`game-menu ${menuOpen ? 'is-open' : ''}`} aria-hidden={!menuOpen}>
      ...
      <footer><span>2DimensionalM</span><i />SWORD ART ONLINE</footer>
    ...
    {view === 'table' && <section className="archive-page">
      ...
      <div className="filter-deck"><label className="search-filter"><span>⌕</span><input value={tableQuery} onChange={(e) => setTableQuery(e.target.value)} placeholder="搜索标题、描述、类型或地点…" /></label>
        ...
        <label><span>TYPE</span><select value={tableType} onChange={(e) => setTableType(e.target.value)}><option value="all">全部类型</option>{settings.taskTypes.map((type) => <option key={type.value}>{type.value}</option>)}</select></label>
